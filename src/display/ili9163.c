...
void lcdDrawPictogram(const uint8_t pictogram[], uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint16_t fgColour, uint16_t bgColour)
{
	...
	// Plot the font data
	int bytesCount = w*h/8;
	for (int byte = 0; byte < bytesCount; byte++) {
		for (int bit = 7; bit >= 0; bit--) {
			if (pictogram[byte] & 1 << bit)
